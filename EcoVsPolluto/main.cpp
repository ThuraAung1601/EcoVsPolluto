...
int main()
{
	...
	while (window.isOpen())
	{
		...
		{
			if (!fullScreen) {
				window.create(sf::VideoMode(windowSize.x, windowSize.y), "EcoVsPolluto", sf::Style::Fullscreen);
				gameManager.ResizeView(window, view, 512.0f);
				background.setTextureRect(sf::IntRect(0, 0, background.getTexture()->getSize().x * 3, background.getTexture()->getSize().y));
				windowWidth = window.getSize().x;
				fullScreen = true;
		...
		{
			if (fullScreen) {
				window.create(sf::VideoMode(1024.0f, 768.0f), "EcoVsPolluto", sf::Style::Resize | sf::Style::Close);
				gameManager.ResizeView(window, view, 512.0f);
				background.setTextureRect(sf::IntRect(0, 0, background.getTexture()->getSize().x * 3, background.getTexture()->getSize().y));
				windowWidth = window.getSize().x;
				fullScreen = false;
