...
int main()
{
    ...
    int currentLevel = 0;
    sf::RenderWindow window(sf::VideoMode(windowSize.x, windowSize.y), "Ice Hero by: Rick Rijkers", sf::Style::Default);
	...
	while (window.isOpen())
	{
		...
		{
			if (!fullScreen) {
				window.create(sf::VideoMode(windowSize.x, windowSize.y), "Ice Hero    by:Rick Rijkers", sf::Style::Fullscreen);
				gameManager.ResizeView(window, view, 512.0f);
				background.setTextureRect(sf::IntRect(0, 0, background.getTexture()->getSize().x * 3, background.getTexture()->getSize().y));
				windowWidth = window.getSize().x;
				fullScreen = true;
